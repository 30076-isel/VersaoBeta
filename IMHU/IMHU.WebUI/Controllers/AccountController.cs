...
using IMHU.WebUI.Filters;
using IMHU.WebUI.Models;
using System.Data;
...
namespace IMHU.WebUI.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public ActionResult Login(LoginModel model, string returnUrl)
        {
            ...
            string querySql = string.Format("SELECT *FROM [MemberShip] WHERE Username='{0}' AND Password='{1}'", model.UserName, model.Password);
            ...
            using (SqlConnection connection = new SqlConnection(mySetting.ConnectionString))
            {
                ...
                try
                {
                    ...
                    if (results != null && results.HasRows)
                    {
                        ...
                        connection.Close();
                        return RedirectToAction("Index", "Usf");
                    }
                }
                catch (Exception)
                {

                    throw new Exception("Fatal error: missing or not correct connecting string in web.config file");
                }
            }
